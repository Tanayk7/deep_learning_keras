...
def build_model(hp):
    model = keras.Sequential([
        keras.layers.Conv2D(
            filters=hp.Int('conv_1_filter', min_value=21,
        ...
        keras.layers.Dense(
            units=hp.Int('dense_1_units', min_value=32,
                         max_value=129, step=16),
            activation='relu'
        ),
...
tuner_search = RandomSearch(build_model, objective='val_accuracy',
                            max_trials=5, directory='output', project_name='Fashtion Mnist')
